<template>
    <div>
        <b-modal id="modal-create-block" class="py-4" ref="modal" :title="$t('objects.create.new_block')" size="lg" hide-footer hide-header-close no-close-on-backdrop>
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-3 ml-2 d-flex flex-column justify-content-end align-items-end">
                                    <div class="mb-3" v-if="disabled.btn_save">
                                        <button type="button" @click="createApartments" :disabled="!block.floor ? true : false" class=" btn btn-primary">
                                            {{ $t('create') }}
                                        </button>
                                    </div>
                                </div>
        ...
        methods: {
            ...
            removeBlock() {
                this.$swal({
                    ...
                    if (result.value) {
                        this.axios.delete(process.env.VUE_APP_URL + '/v2/objects/block/' + this.block.id , this.header).then((response) => {
