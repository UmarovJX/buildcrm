...
export default {
  ...
  computed: {
    ...
    labelApartmentSelection() {
      if (this.showingListType.type === 'plan')
        return 'name'
      return 'number'
    }
  },
  ...
  methods: {
    ...
    initializer() {
      ...
      this.price = price
      this.floor = floor
      this.currency_type = this.sumOptions.find(option => option.value === currency_type)
      this.showingListType = this.typeOptions.find(option => option.type === type)
    },
    async getApartmentsListByType() {
      ...
      if (apartmentNumber) {
        ...
        const getApartments = async () => {
          ...
          await api.promo.fetchApartments({objectID, blockID, apartmentNumber})
              .then((response) => {
        ...
        const getPlans = async () => {
          this.apartmentsByType = []
          await api.promo.fetchPlans(objectID, blockID, apartmentNumber)
              .then((response) => {
                this.apartmentListByType = response.data
              })
              .catch((error) => this.toastedWithErrorCode(error))
    ...
    initializeList() {
      if (this.block.history || this.block.clone) {
        const type = this.block.types.find(type => type.id === this.inputVariable.id)
        this.apartmentsByType = this.apartmentListByType.filter(listType => {
          const hasInHistory = type.values
              .findIndex(value => value === listType.id)
    ...
    saveEnteredData() {
      const {showingListType, price, currency_type, apartmentsByType, inputVariable} = this
      const values = apartmentsByType.map(apartmentByType => apartmentByType.id)
