<template>
    <div class="apartment">
        ...
        <div class="apartment__variant">
            ...
            <div class="apartment__info">Скидка: <span>{{ discount.discount }}% - {{ getDiscount() | number('0,0', { thousandsSeparator: ' ' }) }} {{ $t('ye') }}</span></div>
            <div class="apartment__info">
                Первый взнос - <span>{{ getPrepay() | number('0,0', { thousandsSeparator: ' ' }) }} {{ $t('ye') }}</span>
            </div>

            <div class="apartment__info" v-if="getDebt() > 0">
                Ежемесячный: <span>6 месяцев {{ getMonth() | number('0,0', { thousandsSeparator: ' ' }) }} сум </span>
            </div>

            <div class="apartment__info" v-if="getDebt() > 0">
                Остаток: <span>{{ getDebt() | number('0,0', { thousandsSeparator: ' ' }) }} {{ $t('ye') }}</span>
            </div>

            <div class="apartment__info">Итого: <span>{{ getTotal() | number('0,0', { thousandsSeparator: ' ' }) }} {{ $t('ye') }}</span></div>
        </div>
    </div>
</template>
