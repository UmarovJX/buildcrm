...
export default {
  ...
  methods: {
    ...
    searchByFilterField() {
      const object_id = this.filter.object_id ? this.filter.object_id.map(obj => obj.id) : null
      const sortingQuery = Object.assign({}, this.query, {...this.filter, object_id})
      ...
      for (let property of Object.keys(this.filter)) {
        ...
        if (property === 'object_id' && query) {
          if (isPrimitiveValue(query)) {
            const cObj = this.objectOptions.find(opObj => opObj.id === parseInt(query))
            if (cObj) {
              this.filter[property] = [cObj]
            }
          } else {
            this.filter[property] = query.map(value => {
              const obj = this.objectOptions.find(opObj => opObj.id === parseInt(value))
              if (obj) {
                return obj
              }
              return value
