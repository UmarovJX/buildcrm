...
Shape.Properties = class Properties extends Style.Properties {
  ...
  _getValidChildren(t) {
    ...
    const bodyPr = this.wXml.$1('bodyPr')
    if (bodyPr) {
      for (
        let i = 0, attrs = bodyPr.attributes, len = attrs.length;
        i < len;
        i++
  ...
  lnRef(x) {
    return phClr(
      this.wDoc.getFormatTheme().line(x.attr('idx')),
      this.solidFill(x),
    )
  }
  ...
  fillRef(x) {
    return phClr(
      this.wDoc.getFormatTheme().fill(x.attr('idx')),
      this.solidFill(x),
    )
  }
