...
class Command {
  ...
  nextUntil(seperators) {
    ...
    const node = this.data.substring(0, i).trim()
...
class Switch extends Command {
  constructor(cmd) {
    ...
    if (cmd.length > 1 && this.type != '*' && cmd.charAt(1) != ' ') {
      ...
      this.type = '!'
    } else if (this.data.length > 1) this.data = this.data.substring(1).trim()
    else this.data = ''
    this.__removeQuote()
  }
