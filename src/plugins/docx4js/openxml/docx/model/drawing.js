...
Drawing.SpProperties = {
  ...
  solidFill(x) {
    const elColor = x.firstChild
    let color = this.asColor(elColor.attr('val'))
    let t
  ...
  gradFill(x) {
    const type = x.$1('lin,path')
    const o = this.asObject(type)
    const stops = []
    ...
    o.path
      && (o.rect = this.asObject(type.firstChild, x => parseInt(x) / 1000))
  ...
  prstGeom(x) {
    const px = this.pt2Px
    const w = px(this.world.width)
    const h = px(this.world.height)
    switch (x.attr('prst')) {
      case 'leftBrace':
        return {
          shape: 'path',
  ...
  custGeom(x) {
    ...
    for (
      ...
      switch (a.localName) {
        case 'moveTo':
          path.push(
            `M ${px(a.firstChild.attr('x'))} ${px(a.firstChild.attr('y'))}`,
          )
          break
        case 'lnTo':
          path.push(
            `L ${px(a.firstChild.attr('x'))} ${px(a.firstChild.attr('y'))}`,
          )
          break
          break
        case 'cubicBezTo':
          path.push(
            `L ${
              px(a.childNodes[0].attr('x'))
            } ${
              px(a.childNodes[0].attr('y'))}`,
          )
          path.push(
            `Q ${
              px(a.childNodes[1].attr('x'))
            } ${
              px(a.childNodes[1].attr('y'))
            } ${
              px(a.childNodes[2].attr('x'))
            } ${
              px(a.childNodes[2].attr('y'))}`,
          )
          break
      }
    }
