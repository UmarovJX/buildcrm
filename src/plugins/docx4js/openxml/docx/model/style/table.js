...
export default class Table extends Style {
  parse(factories) {
    ...
    const TableStyle = this.constructor
    for (
      let styles = this.wXml.$('tblStylePr'), len = styles.length, i = 0;
      i < len;
      i++
    ) {
      const model = new TableStyle(styles[i], this.wDoc, this)
      model.id = this.id
...
Table.Properties = class Properties extends Style.Properties {
  ...
  tblCellMar(x) {
    const value = {}
    for (
      var borders = x.childNodes, i = 0, len = borders.length, v;
      i < len;
      i++
    ) {
      borders[i].nodeType == 1
        && (value[borders[i].localName] = this.pt2Px(
          this.asPt(borders[i].attr('w:w')),
        ))
    }
    return value
  }
  ...
  tblLook(x) {
    return this.asObject(x, x => parseInt(x))
  }
...
Table.RowProperties = class RowProperties extends Style.Properties {
  cnfStyle(x, t) {
    return Object.keys((t = this.asObject(x)))
      .map(a => t[a] == '1' && StyleNameMap[a])
      .filter(a => a)
  }
