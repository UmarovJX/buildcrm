...
class Properties extends Style.Properties {
  ...
  spacing(x) {
    const r = this.asObject(x)
    const o = {}

    if (!r.beforeAutospacing && r.beforeLines) o.top = this.pt2Px(this.asPt(r.beforeLines))
    else if (r.before) o.top = this.pt2Px(this.asPt(r.before))

    if (!r.afterAutospacing && r.afterLines) o.bottom = this.pt2Px(this.asPt(r.afterLines))
    else if (r.after) o.bottom = this.pt2Px(this.asPt(r.after))
  ...
  pBdr(x) {
    const r = {}
    const bdr = Inline.Properties.prototype.bdr.bind(this)
    Array.from(x.childNodes).forEach(
      a => a.localName && (r[a.localName] = bdr(a)),
    )
    return r
  }
