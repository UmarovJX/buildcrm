...
export default class NumberingDefinition extends Style {
  constructor(wXml) {
    ...
    this.name = this.id = this.constructor.asStyleId(
      wXml.attr('w:abstractNumId'),
    )
    this.wDoc.style.set(this)
    const link = wXml.$1('numStyleLink')
  ...
  getLabel(...indexes) {
    const [level] = indexes[indexes.length - 1]
    indexes = new Map(indexes)
    const { lvlText } = this.levels.get(level).values
    const label = lvlText.replace(/%(\d+)/g, (a, index) => {
