...
export default class Inline extends Style {
  ...
  static Properties = class extends Style.Properties {
    ...
    rFonts(x) {
      let t; let ascii; let
        asia
      if ((t = x.attr('w:ascii'))) ascii = t
      else if ((t = x.attr('w:asciiTheme'))) ascii = this.wDoc.getFontTheme().get(t)

      if ((t = x.attr('w:eastAsia'))) asia = t
      else if ((t = x.attr('w:eastAsiaTheme'))) asia = this.wDoc.getFontTheme().get(t)
      if (ascii || asia) return { ascii, asia }
    }
    ...
    color(x) {
      return this.asColor(
        x.attr('w:val') || this.wDoc.getColorTheme().get(x.attr('w:themeColor')),
      )
    }
