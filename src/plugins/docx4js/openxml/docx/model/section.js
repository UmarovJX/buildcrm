...
export default class section extends Model {
  constructor(wXml, wDoc, mParent) {
    super(...arguments)
    mParent.content.pop()
    this.wFirst = mParent.content.length
      ? mParent.content[mParent.content.length - 1].wLast.nextSibling
      : mParent.wXml.firstChild
  ...
  _iterateHeaderFooter(visitorFactories, refType) {
    for (
      let refs = this.wXml.$(`${refType}Reference`), i = 0, len = refs.length;
      i < len;
      i++
    ) {
      const part = (this.wDoc.parseContext.part.current = this.wDoc.getRel(
        refs[i].attr('r:id'),
      ))
      const model = new (require(`./${refType}`))(
        part.documentElement,
        this.wDoc,
        this,
        refs[i].attr('w:type'),
      )
