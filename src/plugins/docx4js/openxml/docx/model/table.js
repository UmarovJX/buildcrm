...
export default class table extends require('../model') {
  ...
  getStyleId(a) {
    return (
      this._val('>tblPr>tblStyle')
      || ((a = this.wDoc.style.getDefault(TableStyle.type)) && a.id)
    )
  }
  ...
  getColWidth() {
    const { asPt } = TableStyle.Properties.prototype
    const { pt2Px } = TableStyle.Properties.prototype
    const widths = []
    let sum = 0
    for (
