...
const fields = {
  hyperlink, date, ref, pageref, toc, page,
}
export default class fieldBegin extends require('../model') {
  ...
  _iterate(f, factories, visitors) {
    ...
    this.end = function (endModel, endVisitors) {
      ...
      const instruct = this.commands.join('').trim()
      const index = instruct.indexOf(' ')
      const type = (
        index != -1 ? instruct.substring(0, index) : instruct
      ).toLowerCase()

      this.field = this.constructor.factory(instruct, this.wDoc, this, type)
      if (this.field) this.field = new basic(instruct, this.wDoc, this, type)
