...
export default class list extends require('./paragraph') {
  constructor() {
    ...
    const numId = (t => {
      let numId = (t = this.wXml.$1('>pPr>numPr'))
        && (t = t.$1('numId'))
        && (t = t.attr('w:val'))
    ...
    const level = (t => ((t = this.wXml.$1('>pPr>numPr>ilvl')) ? t.attr('w:val') : '0'))()
  ...
  parse() {
    const { numbering } = this.wDoc.parseContext
  ...
  getLabel() {
    return this.wDoc.parseContext.numbering.getLabel(
      this.getNumberingId(),
      parseInt(this.getLevel()),
    )
  }
