export default class model extends require('../parser') {
  ...
  parse(visitFactories) {
    const visitors = []
    const paramizedVisitFactories = []
    $.map(
      visitFactories,
      visitFactory => {
        const visitor = visitFactory(this)
    ...
    if (visitors.length > 0) {
      const factory = this.wDoc.factory.bind(this.wDoc)
      this._iterate(
        wXml => factory(wXml, this.wDoc, this).parse(paramizedVisitFactories),
        paramizedVisitFactories,
        visitors,
      )
    }
  ...
  _iterate(f, paramizedVisitFactories) {
    for (
      let i = 0,
        children = this._getValidChildren(),
