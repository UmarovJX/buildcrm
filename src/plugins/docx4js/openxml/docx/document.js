...
export default class document extends require("../document") {
  constructor() {
    ...
    var rels = this.rels,
      builtIn =
        "settings,webSettings,theme,styles,stylesWithEffects,fontTable,numbering,footnotes,endnotes".split(
          ","
        );
  ...
  parse(visitFactories) {
    ...
    var roots = this.content.parse(
      $.isArray(visitFactories) ? visitFactories : $.toArray(arguments)
    );
    this.release();
