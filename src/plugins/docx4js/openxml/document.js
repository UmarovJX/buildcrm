...
import DocumentV2 from '../document'

export default class Document extends DocumentV2 {
  constructor() {
    super(...arguments)
    const rels = (this.rels = {})
    $.each(new Part('', this).rels, (id, rel) => {
  ...
  getPart(name) {
    const part = this.parts[name] || ((name = this.rels[name]) && this.parts[name])
    if (!part) return null
  ...
  static createVisitorFactory(factory, opt) {
    ...
    if (opt) {
      const _raw = factory
      factory = function () {
        const converter = _raw.apply(null, arguments)
