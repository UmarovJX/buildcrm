...
function asStyle(x) {
  const a = []
  for (const i in x) !$.isFunction(x[i]) && a.push(`${i.replace(AZ, r)}:${x[i]}`)
  return a.join(';')
}
...
export default class Shape extends Converter {
  ...
  convertStyle(el) {
    ...
    const pathStyle = { stroke: 'black', strokeWidth: 2, fillOpacity: 0 }
    const bgStyle = this.makeBackgroundStyle()
    super.convertStyle(...arguments)
    const style = this.wordModel.getDirectStyle()
    const propConverter = new this.constructor.Properties(
      el.style,
      this,
      pathStyle,
      bgStyle,
    ...
    if (this.path) {
      ...
      const bgImage = el.style.background
      const { grad } = pathStyle
      ...
      const svgImage = `url(${this.doc.asImageURL(svg)})`
  ...
  makeBackgroundStyle() {
    ...
    const id = `shape${this.doc.uid()}`
    this.content.setAttribute('id', id)
    const style = this.doc.createStyle(`#${id}::before`)
    style.content = '""'
    style.zIndex = -1
...
Shape.Properties = class Properties extends Style.Properties {
  ...
  path(x, t) {
    switch (x.shape) {
      case 'line':
        this.parent.path = `<line x1="0" y1="0" x2="${
          this.world.width
        }pt" y2="${
          this.world.height
        }pt"`
  ...
  vert(x) {
    ...
    const delta = (this.world.width - this.world.height) / 2
