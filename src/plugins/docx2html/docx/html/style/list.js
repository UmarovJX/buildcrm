...
export default class List extends Style {
  ...
  _getPropertiesConverter(category) {
    ...
    const info = category.split(' ')
    const level = parseInt(info[0])
    const type = info.length == 1 ? 'list' : info[1]
    let style = this.levelStyles[level]
    const levelSelector = `.${cssID(this.wordModel.id)}[level="${level}"]`
    ...
    switch (type) {
      case 'inline':
        style.inline = new Inline.Properties(
          this.doc.createStyle(`${levelSelector}>li>p>.marker:before`),
        )
        break
      case 'paragraph':
        style.paragraph = new this.constructor.Pr(
          this.doc.createStyle(`${levelSelector}>li>p`),
          this,
          levelSelector,
        )
        break
      case 'list':
        style.list = new this.constructor.Properties(
          this.doc.createStyle(`${levelSelector}>li>p>.marker:before`),
          this,
          levelSelector,
          `${cssID(this.wordModel.id)}_${level}`,
          level,
        )
        break
    }
...
List.Pr = class Pr extends Paragraph.Properties {
  ...
  ind(x) {
    ...
    x.hanging = hanging
    x.hanging
      && (this.doc.createStyle(`${this.levelSelector}>li>p>.marker`).left = `${-x.hanging}px`)
  }
}
