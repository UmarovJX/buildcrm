...
export default class StyleConverter extends Converter {
  constructor() {
    ...
    parentStyle &&
      this.doc.stylePath(cssID(this.wordModel.id), cssID(parentStyle.id));
  }
...
StyleConverter.Properties = class Properties {
  ...
  _border(border) {
    if (border.val == "none" || border.val == "nil") return "0";
      ...
      return (
        (border.sz < 1 && border.sz > 0 ? 1 : border.sz) +
        "pt " +
        (Lines.indexOf(border.val.toLowerCase()) != -1 ? border.val : "solid") +
        " " +
        (border.color || "")
      );
  }
  ...
  lineStyle(x) {
    ...
    else if (x.indexOf("dash") != -1) return "dashed";
    else if (x.indexOf("double") != -1 || x.indexOf("gap") != -1)
      return "double";
    else if (x.indexOf("emboss") != -1) return "ridge";
    else if (x.indexOf("grave") != -1) return "groove";
    else if (x.indexOf("outset") != -1) return "outset";
    else if (x.indexOf("inset") != -1) return "inset";
    else return "solid";
  }
};
