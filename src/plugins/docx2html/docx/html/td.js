...
class Properties extends Style.CellProperties {
  ...
  cnfStyle(names) {
    ...
    const { PrioritiziedStyles } = Style.prototype
    names = names.sort(
      (a, b) => PrioritiziedStyles.indexOf(a) - PrioritiziedStyles.indexOf(b),
    )
    const level = PrioritiziedStyles.indexOf(names[names.length - 1])

    Td.addClass(this.parent.content, names.join(' '))
    for (let i = 0; i < level; i++) this.parent.content.setAttribute(`x${i}`, 1)
  }
}
