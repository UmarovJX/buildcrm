        ...
        async getBlockItems(blocks) {
            ...
            const setObjectMap = (_b) => {
                if (this.gridApartments.length) {
                    const idx = this.gridApartments.findIndex(_obj => _obj.id === _b.building.id)
                    if (idx !== -1) {
                ...
                if (this.hasQuery) {
                    ...
                    this.chessApartments = this.filterItems(this.query, this.gridApartments)
                } else {
            ...
            async function fetchGrid() {
                await api.objectsV2.getOptimizeApartments(object, blocks[idx])
                    .then(({data}) => {
                        setObjectMap(data)
                        idx++
