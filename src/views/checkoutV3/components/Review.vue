...
import ChPlanDetails from "@/views/checkoutV3/components/PlanDetails";
...
export default {
  ...
  props: ["client", "countryList", "clientTypeList", "order", "total"],
  data() {
    return {
      companyTypes: [],
    };
  ...
  computed: {
    ...
    clientDetails() {
      ...
      if (this.client.subject === "legal") {
        const findCompanyType = this.companyTypes.find(
          (typeCtx) => typeCtx.id === this.client.attributes.company_type_id
        );
        ...
        details = [
          ...
          {
            title: "bank",
            content: this.client.attributes.bank_name,
          ...
          {
            title: "company_name",
            content: this.client.attributes.name,
            icon: "apartment",
          },
          {
            title: "account_number",
            content: this.client.attributes.payment_number,
            icon: "account_balance_wallet",
          },
          {
            title: "mfo",
            content: this.client.attributes.mfo,
            icon: "numbers",
          },
          {
            title: "inn",
            content: this.client.attributes.inn,
            icon: "numbers",
          },
          {
            title: "ndc",
            content: this.client.attributes.nds,
            icon: "numbers",
          },
          {
            title: "fax",
            content: this.client.attributes.fax,
            icon: "fax",
          },
          {
            title: "legal_address",
            content: this.client.attributes.legal_address,
            icon: "location_on",
          },
        ];
      } else {
        ...
        const name = {
          l: this.client.attributes.last_name[typography],
          f: this.client.attributes.first_name[typography],
          m: this.client.attributes.middle_name[typography],
        };

        const fullName = `${name.l} ${name.f} ${name.m}`;

        const country = this.countryList.find(
          (cty) => cty.id === this.client.attributes.country_id
        );
        ...
        details = [
          ...
          },
          {
            ...
            content: this.client.attributes.passport_series,
            icon: "contact_page",
          },
          {
            title: "birth_day",
            content: this.client.attributes.date_of_birth,
            icon: "cake",
          },
          {
            title: "fio_full",
          ...
          },
          {
            ...
            content: this.client.language,
            icon: "language",
          },
          {
            title: "checkout.address_line",
            content: this.client.attributes.address_line,
            icon: "home_pin",
          },
        ];
      }
      ...
      details.push({
        title: "phone",
        content: this.client.phones[0].phone,
        icon: "call",
      });

      if (
        this.client.phones.length > 1 &&
        isNUNEZ(this.client.phones[1].phone)
      ) {
        details.push({
          title: "additional_phone_number",
          content: this.client.phones[1].phone,
          icon: "call",
        });
      }

      if (isNUNEZ(this.client?.email)) {
        details.push({
          title: "email",
          content: this.client.email,
          icon: "mail",
        });
      }

      if (isNUNEZ(this.client?.additional_email)) {
        details.push({
          title: "additional_email",
          content: this.client.additional_email,
          icon: "mail",
        });
      }
  ...
  <div>
    ...
    <div class="apn-result">
      <div
        ...
        :key="ord.apartment.uuid"
        class="apn-result__item"
      >
        <section-title
          :bilingual="true"
          :title="$t('apartment') + ' ' + ord.apartment.number"
          class="km-b-2"
        />
        <div class="apn-result__item-content">
          <ch-plan-details :order="ord" />
          <ch-payment-result
            :order="ord"
            :result="true"
