...
export default {
        ...
        },
        {
          ...
          formatter: (client) => {
            if (client.attributes) {
              return client.attributes.last_name.lotin + ' ' + client.attributes.first_name.lotin
            }
            return ''
          },
          sortable: true
  ...
  methods: {
    ...
    getFullName(client) {
      try {
        if (client && Object.keys(client).length && !isUndefinedOrNullOrEmpty(client.attributes)) {
          const {first_name, last_name, middle_name} = client.attributes
          const language = this.$i18n.locale === 'ru' ? 'kirill' : 'lotin'
          return last_name[language] + ' ' + first_name[language] + ' ' + middle_name[language]
        }
        return ''
      } catch (e) {
        console.error(e)
    ...
    async getMoreDataAboutDebt() {
      ...
      await api.debtorsV2.getMoreDataAboutDebt(this.debtorViewModalItem.uuid)
          .then((response) => {
    ...
    async fetchItems(params) {
      ...
      return await api.debtorsV2.fetchDebtorsList(params)
