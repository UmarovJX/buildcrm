...
export default {
  ...
  methods: {
    ...
    async initDebtorUi() {
      const { starterPoint: monthStarter, endPoint: monthEnd } =
        this.getDateDistance(41, this.month.starter);

      const { starterPoint: weekStarter, endPoint: weekEnd } =
    ...
    changeViewType(type) {
      if (type !== this.typeOfView) {
        this.typeOfView = type;
        const starter =
          this.query.starter_moment ?? formatDateToYMD(new Date());
...
<template>
  <div>
    ...
    <!--  DEBTOR VIEW MODAL  -->
    <base-right-modal ref="debtor-view-modal" @show="getMoreDataAboutDebt">
