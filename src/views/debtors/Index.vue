...
export default {
  ...
  data() {
    ...
    const typeOfView = query.type_of_view ?? 'list' /* list / month / week / day */
  ...
  methods: {
    ...
    getClientMajorPhone(client) {
      if (hasOwnProperty(client, 'phones')) {
        ...
        phones = phones.filter(p => (
          isNotUndefinedNullEmptyZero(p.phone)
    ...
    getFullName(client) {
      try {
        if (
          ...
          && !isUndefinedOrNullOrEmpty(client.attributes)
        ) {
          const { first_name, last_name, middle_name } = client.attributes
    ...
    async getMoreDataAboutDebt() {
      ...
      await api.debtorsV2
        .getMoreDataAboutDebt(this.debtorViewModalItem.uuid)
        .then(response => {
    ...
    getDateDistance(distanceLength, ymd) {
      ...
      const getStarterPoint = dateConvertor(moment).setDate(
        getMomentDate - countOfBackDate,
      )
      const starterPoint = dateConvertor(getStarterPoint)
      const getStarterPointDate = starterPoint.getDate()
      const getEndPoint = dateConvertor(starterPoint).setDate(
        getStarterPointDate + distanceLength,
      )
    ...
    initMonthItems(items) {
      ...
      for (let i = 1; i < 42; i++) {
        const nextMoment = starterPoint.date.setDate(
          starterPoint.date.getDate() + 1,
        )
    ...
    changeCurrentPage(page) {
      ...
      if (this.typeOfView) this.initDebtorUi()
    },
    ...
    changeViewType(type) {
      if (type !== this.typeOfView) {
        ...
        const { year, month, dayOfMonth } = dateProperties(
          dateConvertor(starter),
        )
        if (type === 'day') {
          ...
          if (type === 'month') {
            ...
            this.week.starter = formatDateToYMD(
    ...
    changeCalendarDate(lastDate) {
      const {
        year, month, dayOfMonth, ymd,
      } = dateProperties(lastDate)
      switch (this.typeOfView) {
        case 'month': {
    ...
    filterDebts({
      date, price_from, price_to, client_type, object_id,
    }) {
      const type = this.typeOfView
      const query = {
        price_from, price_to, client_type, object_id,
      }
  ...
  <div>
    ...
    <template v-if="showListOfDebtorsTable">
      <b-table
        ...
        v-model:sort-desc="table.sortDesc"
        show-empty
        borderless
        responsive
        sticky-header
      ...
      >
        ...
        <template #cell(client)="tableItems">
          <span
            v-if="
              tableItems.item.order.friends &&
                isNotUndefinedNullEmptyZero(tableItems.item.client.attributes)
            "
          >
            <span class="friend">
    ...
    >
      <template #modal-title>
        <span class="pl-3 modal-title">
          ...
          <span class="color-violet-600">{{
            debtorViewModalItem.order.contract
          }}</span>
        </span>
      </template>
      ...
      <template v-else>
        ...
        <div class="contract-details">
          <!--    CONTRACT DETAILS    -->
          <p class="contract-details-title">
            {{ $t("contract_details") }}
          </p>
          <!--  CLIENT INFORMATION      -->
          <output-information
          ...
          <output-information
            :property="`${$t('phone')}`"
            :value="
              getClientMajorPhone(phonePrettier(debtorViewModalItem.client))
            "
            class="mt-4 mb-4"
          />
          <!--   CLIENT TYPE     -->
          ...
          <output-information
            ...
            :property="`
              ...
              `"
            :value="
              pricePrettier(debtorViewModalItem.order.installment_payment)
            "
            class="mt-4 mb-4"
          />
        </div>
        ...
        <div class="contract-details">
          ...
          <output-information
            ...
            :value="
              pricePrettier(
                subtractResult(
                  debtorViewModalItem.amount,
                  debtorViewModalItem.amount_paid
                )
              )
            "
            class="mt-4 mb-4"
      ...
      <template #modal-footer>
        <base-button
          ...
          @click="
            $router.push({
              name: 'contracts-view',
              params: { id: debtorViewModalItem.order.uuid },
            })
          "
        />
      </template>
