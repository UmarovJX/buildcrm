...
export default {
  ...
  findApmIdx: (state) => (id) =>
    state.apartments.findIndex((apn) => apn.id === id),
  discountType: (state, gts) => (idx) => gts.getApm({ idx }).calc.discount.type,
  discountAmount: (state, gts) => (idx) =>
    gts.getApm({ idx }).calc.discount.amount,
  apartmentArea: (state, gts) => (idx) => gts.getApm({ idx }).plan.area,
  apartmentPricePerSquare: (state, gts) => (idx) =>
    gts.getApm({ idx }).price_m2,
  isDiscountOtherType: (state, gts) => (idx) =>
    gts.getApm({ idx }).calc.discount.id === "other",
  getPrepay: (state, gts) => (idx) => gts.getApm({ idx }).calc.prepay,
  getMonth: (state, gts) => (idx) =>
    gts.getApm({ idx }).calc.monthly_payment_period,
  discount:
    (state, gts) =>
    ({ idx, discountId }) => {
  ...
  getInitialPayments: (state, gts) => (idx) =>
    gts.getApm({ idx }).calc.initial_payments,
  getCreditMonths: (state, gts) => (idx) =>
    gts.getApm({ idx }).calc.credit_months,
  getMonthlyEdit: (state, gts) => (idx) =>
    gts.getCreditMonths(idx).filter((monthly) => monthly.edit),
  getInitialEdit: (state, gts) => (idx) =>
    gts.getInitialPayments(idx).filter((monthly) => monthly.edit),
  initiallyTotal: (state, gts) => (idx) =>
    gts.getInitialPayments(idx).reduce((a, b) => a + b.amount, 0),
  creditMonthTotal: (state, gts) => (idx) =>
    gts.getCreditMonths(idx).reduce((a, b) => a + b.amount, 0),
  getMonthlyEditTotalPrice: (state, gts) => (idx) =>
    gts.getMonthlyEdit(idx).reduce((a, b) => a + b.amount, 0),
  getInitialEditTotalPrice: (state, gts) => (idx) =>
    gts.getInitialEdit(idx).reduce((a, b) => a + b.amount, 0),
  uneditedInitial: (state, gts) => (idx) =>
    gts.getInitialPayments(idx).filter((inMonth) => !inMonth.edit),
  uneditedMonthly: (state, gts) => (idx) =>
    gts.getCreditMonths(idx).filter((crMonth) => !crMonth.edit),
  ...
  calculateTotalPriceByPromo: (state, gts) => (idx) =>
    gts.calculateTotalPriceByFixed(idx),
  calculateTotalPriceByAddition: (state, gts) => (idx) =>
    gts.calculateTotalPriceByFixed(idx),
  calculateTotalPriceByDefault: (state, gts) => (idx) => {
    const apartment = gts.getApm({ idx });
    if (gts.getPrepay(idx) === 100 || apartment.discount.prepay === 100) {
      if (gts.isDiscountOtherType(idx)) {
  ...
  totalForAll: (state, gts) =>
    gts.gtsApartments.reduce((a, b) => a + b.calc.total, 0),
};
