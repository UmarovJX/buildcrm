...
export function useReport({ immediate = false } = { immediate: false }) {
  ...
  const typeOptions = computed(() => typeOptionsRaw.value.map(el => ({
    value: el.value,
  ...
  function checking({ ids }) {
    timer.value = setInterval(async () => {
      ...
      if (isAnyFinished) {
        ...
        if (isAnyFailed) {
          const isCurrentFailed = checkRsp.data.result.findIndex(item => item.id === retryingFileId.value && item.status === 'failed')
