...
export default {
  ...
  reInitCalc({ getters, dispatch, commit }, payment) {
    if (payment.type === "initial") {
      ...
      if (getters.getMonth) {
        const excessAmount =
          (getters.getTotal - getters.initiallyTotal) / getters.getMonth;
      ...
      if (getters.uneditedMonthly.length) {
        let monthly =
          (getters.getMonthlyTotalPrice - getters.getMonthlyEditTotalPrice) /
        ...
        if (getters.uneditedInitial.length) {
          let initially =
            (remainToInitial - getters.getInitialEditTotalPrice) /
            getters.uneditedInitial.length;
          dispatch("changeInitiallyByEdit", { initially });
        } else {
          let initialMonthly =
            remainToInitial / getters.getInitialPayments.length;
