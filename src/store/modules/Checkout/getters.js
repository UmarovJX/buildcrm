export default {
  ...
  apartmentPricePerSquare: (state) => (index) =>
    state.apartments[index].price_m2,
  ...
  getMonthlyEdit: (state, getters) =>
    getters.getCreditMonths.filter((monthly) => monthly.edit),
  getInitialEdit: (state, getters) =>
    getters.getInitialPayments.filter((monthly) => monthly.edit),
  initiallyTotal: (state, getters) =>
    getters.getInitialPayments.reduce((a, b) => a + b.amount, 0),
  creditMonthTotal: (state, getters) =>
    getters.getCreditMonths.reduce((a, b) => a + b.amount, 0),
  getMonthlyEditTotalPrice: (state, getters) =>
    getters.getMonthlyEdit.reduce((a, b) => a + b.amount, 0),
  getInitialEditTotalPrice: (state, getters) =>
    getters.getInitialEdit.reduce((a, b) => a + b.amount, 0),
  uneditedInitial: (state, getters) =>
    getters.getInitialPayments.filter((inMonth) => !inMonth.edit),
  uneditedMonthly: (state, getters) =>
    getters.getCreditMonths.filter((crMonth) => !crMonth.edit),
  calculateTotalPriceByFixed: (state, getters) => {
    return state.apartments.reduce((a, b, index) => {
  ...
  calculateTotalPriceByPromo: (state, getters) =>
    getters.calculateTotalPriceByFixed,
  calculateTotalPriceByAddition: (state, getters) =>
    getters.calculateTotalPriceByFixed,
